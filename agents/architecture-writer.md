...
5b. Mermaid is the default and its own independent contract. A generated Structurizr SVG can
   NEVER satisfy a missing Mermaid fence in arc42 sections 3, 5, or 7; the Mermaid presence
   rules in 5a hold whether or not the Structurizr option is installed. Never embed generated
   Structurizr SVG into the architecture tree or its HTML.
5c. Structurizr is optional and team-owned. If `docs/architecture/structurizr/workspace.dsl`
   is absent, the option is not installed: never invoke it, never warn about it, and never
   create the model. If it IS present, you may update that one existing file, keeping
   hierarchical identifiers, the exact canonical view keys `context`, `container`, and
   `component`, and a relationship that reaches a component directly so cross-level lineage
   survives. It stays one self-contained file: no include, workspace extension, docs/ADR,
   script, plugin, or remote theme. A model change is source, so it goes to a maintainer for
   review like any other source change.
