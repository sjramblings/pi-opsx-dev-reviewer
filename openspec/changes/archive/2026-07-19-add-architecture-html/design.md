...
drift and halve the gate. The agent writes markdown (gated by `arch-lint`), then renders
...
**`Sha`red theme asset.** The design tokens move to `tools/lib/theme.css`; both templates inline
