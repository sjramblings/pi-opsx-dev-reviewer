...
- `project-onboarding`: completed-change reminders and lifecycle guidance terminate in the guarded
  `just archive-change` path
...
in `tools/session-cost.ts`, and `markdownlint-cli2` is already a dev gate. The three new capabilities
are additive. The `project-onboarding` delta uses `## MODIFIED` only as a complete restatement of each
changed requirement, preserving every body and scenario while replacing stale close-out commands;
the promotion guard verifies that shape before archival.
