...
- [x] 2.1 **Reproduce, then gate the destructive promotion.** Write the failing test first: build a
...
- [x] 2.2 Add the one-command close-out: a thin `just archive-change change` recipe chaining
...
- [x] 3.1 Open the bounded channel and gate it, in one pass. Add `MAIN_BASH_WRITERS` to
...
- [x] 3.2 **Live shakedown—unit tests are not evidence the pi loader accepted the file.** After
...
- [x] 4.1 Make the protocol and lifecycle surfaces agree in one pass, because they must not
      contradict each other: `prompts/opsx-loop.md` step 3c (the orchestrator records the verdict
      itself via `just record-verdict`, replacing "the developer does this write—you are read-only
      under force-delegate"; keep the tick with the developer and keep ledger-before-tick ordering);
      ...
      `just archive-change <change>`. Point every post-last-task and retro terminal sequence at
      `just archive-change` instead of exposing `just archive-check` or raw `openspec archive`.
      Restate each affected `project-onboarding` requirement in full so its body and every scenario
      survive deterministic promotion.
      files: `prompts/opsx-loop.md`, `prompts/opsx-retro.md`, `openspec/schemas/dev-reviewer/schema.yaml`, `templates/AGENTS.md`, `openspec/changes/add-orchestrator-bookkeeping/{proposal.md,design.md,specs/**}`, `tools/next-lifecycle.test.ts`
      probe: no surface still instructs the developer to append the ledger; lifecycle regression tests reject separate `archive-check` and raw `openspec archive` paths; `bun tools/promote-guard.ts add-orchestrator-bookkeeping`, strict change validation, and `openspec schema validate dev-reviewer` pass; status still resolves all seven artifacts.
      out-of-scope: the developer/reviewer protocol itself.
      parallel: none (must follow 3.1)
      spec: `orchestrator-bookkeeping`

- [x] 4.2 Demote the two prose skills to the gated fallback: `.pi/skills/openspec-archive-change/` and
...
- [x] 5.1 Delegate the `docs` artifact to the `tech-writer` subagent: document the one-command
