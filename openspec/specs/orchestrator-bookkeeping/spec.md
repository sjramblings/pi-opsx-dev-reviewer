# orchestrator-bookkeeping Specification

## Purpose

TBD - created by archiving change add-orchestrator-bookkeeping. Update Purpose after archive.
...
### Requirement: Completed changes use guarded archival

Every completed-change lifecycle surface SHALL direct the operator through `/opsx-retro <change>` and
terminate with `just archive-change <change>`. No lifecycle instruction SHALL expose
`just archive-check` or `openspec archive` as a separate operator command, because either stale path
can bypass or strand the promotion guard.

#### Scenario: Retro terminates in the guarded close-out command

- **WHEN** `/opsx-retro <change>` finishes its ratchet steps
- **THEN** its terminal command is `just archive-change <change>`

#### Scenario: Lifecycle guidance has no stale close-out path

- **WHEN** the loop prompt, retro prompt, schema apply instruction, `templates/AGENTS.md`, and effective
  `project-onboarding` lifecycle contract are read together
- **THEN** each required close-out uses `just archive-change`, with no separate `just archive-check` or
  `openspec archive` instruction
