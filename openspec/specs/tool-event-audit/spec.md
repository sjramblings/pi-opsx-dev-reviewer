# tool-event-audit Specification

## Purpose

Persist blocked tool actions as fail-open runtime evidence and assess that evidence into ranked candidate learnings without weakening guard behavior or Pi load safety.
...
- **WHEN** the ledger cannot be written (for example read-only filesystem)
