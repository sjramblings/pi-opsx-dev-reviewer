/*
 * worktree-canary -- global fail-closed guard for marker-bearing harness sessions.
 *
 * A tracked .harness-marker identifies the harness even in a bare worktree with no
 * project extensions. Valid delegated children remain silent and retain full tools.
 * Every marker-bearing top-level process must carry the current PID handshake.
 *
...
const HALT_REASON =
	...
	"Stop now and return to the primary checkout. Provision a safe worktree with " +
	"just worktree <name>. If this directory was created manually, remove it and recreate " +
	"it through that recipe before restarting pi.";
...
function reportSideChannelFailure(channel: string, error: unknown): void {
	process.stderr.write(
		"worktree-canary: " + channel + " failed after HALT: " + errorMessage(error) + "\n",
	);
}
